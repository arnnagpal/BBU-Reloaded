        ...
        for (team in BBU.getInstance().teamController.allTeams) {
            ...
            if (team.hasBeacon()) {
                if (team.players.size > 0) {
                    lines.add(
                        "&" + team.colour.chatColor.char + team.colour.name.first() + " &f" + name + "&f: &a" + team.players.size.toString() +
                                if (team.players.contains(player)) " &7YOU" else ""
                    )
                } else {
                    lines.add(
                        "&" + team.colour.chatColor.char + team.colour.name.first() + " &f" + name + "&f: &c\u2715" +
                                if (team.players.contains(player)) " &7YOU" else ""
                    )
                }
            } else {
                lines.add(
                    "&" + team.colour.chatColor.char + team.colour.name.first() + " &f" + name.toString() + "&f: &a\u2713" +
                            if (team.players.contains(player)) " &7YOU" else ""
                )
            }
        }
